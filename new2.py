...
def fetchData():
    ...
    try:
        ...
        print("Error occurred during training or evaluation", e)
...
def gatherData(client):
    ...
    if not data_list:
        print("No data available for the current report.")
        return None
...
def run_evaluation(client):
    ...
    for idx, error in enumerate(reconstruction_errors):
        ...
        if error > threshold:
            print(f"Sequence {idx + 1}: Anomaly detected with probability {probability:.2f}%.")
        else:
            print(f"Sequence {idx + 1}: Normal data with probability {probability:.2f}%.")
